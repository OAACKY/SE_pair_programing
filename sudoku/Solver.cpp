...
int Head[maxrow];	// 记录01矩阵每行第一个“1”元素是num个元素中第几个元素
static char str[163];
static int index;
...
void Solver::transform(int table[][9], char ch[]) {
	int index = 0;
	for (size_t i = 0; i < 9; i++)
		for (size_t j = 0; j < 9; j++)
...
void Solver::insertNode(int col, int no) {	// 将no号元素添加到col列里
...
void Solver::remove(int col) {	// 按列标元素移除相关元素
	/* 移除col列标元素 */
